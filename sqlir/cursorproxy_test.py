...
def test_proxy__lazy_query__multiple_rows__doesnt_fail(engine: Engine) -> None:
    ...
    rows = engine.select(Person).fetchall()
...
def test_proxy__when_querying_view_model__column_name_need_not_match_field_name(engine: Engine) -> None:
    class ModelA(Row):
        name: str

        __select_query__ = "SELECT 'Alice' AS not_name"
